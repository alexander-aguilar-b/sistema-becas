...
namespace PruebaMvc.DAL
{
    public class FormularioDAL
    {
        ...
        public void crearFormulario(String cedula, Items items)
        {
            try
            {
        ...
        public void borrarFormulario(String cedula)
        {
            try
            {
                SqlConnection conn = new SqlConnection(connString);
                conn.Open();
                SqlCommand procedure = new SqlCommand("spEliminarFormulario", conn);
                procedure.CommandType = CommandType.StoredProcedure;

                procedure.Parameters.Add("@Cedula", SqlDbType.VarChar, 20).Value = cedula;
